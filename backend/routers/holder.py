...
async def get_my_credential(x_holder_token: str | None = Header(default=None, alias="X-Holder-Token")):
    ...
    return {
        ...
        "grants": [_grant_view(g) for g in share_grants.list_grants(entry.credential_id)],
    }
...
async def create_grant(
    ...
    grant = share_grants.create_grant(entry.credential_id, payload.revealGpa)
...
async def revoke_grant(
    ...
    if not share_grants.revoke_grant(grant_id, entry.credential_id):
